...
const vigenere_1 = require("./vigenere");
describe('Vigenère Cipher', () => {
    it('encrypts classic vigenere correctly', () => {
        const result = (0, vigenere_1.vigenereEncrypt)('ATTACKATDAWN', { key: 'LEMON' });
        expect(result.ciphertext).toBe('LXFOPVEFRNHR');
    });
    it('decrypts classic vigenere correctly', () => {
        const result = (0, vigenere_1.vigenereDecrypt)('LXFOPVEFRNHR', { key: 'LEMON' });
        expect(result.plaintext).toBe('ATTACKATDAWN');
    });
    it('encrypts autokey vigenere correctly', () => {
        const result = (0, vigenere_1.vigenereEncrypt)('ATTACKATDAWN', { key: 'LEMON', variant: 'autokey' });
        // Key:    LEMONATTACKA
        // Text:   ATTACKATDAWN
        // Result: LXFOPKTMDCGN
    ...
    it('decrypts autokey vigenere correctly', () => {
        const result = (0, vigenere_1.vigenereDecrypt)('LXFOPKTMDCGN', { key: 'LEMON', variant: 'autokey' });
        expect(result.plaintext).toBe('ATTACKATDAWN');
    });
    it('preserves casing and punctuation', () => {
        const result = (0, vigenere_1.vigenereEncrypt)('Attack at dawn!', { key: 'LEMON' });
    ...
    it('calculates IC correctly (approx English)', () => {
        ...
        const ic = (0, vigenere_1.calculateIC)(text);
